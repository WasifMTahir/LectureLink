...
const binaryUpdate =(x)=>{
	...
	socket.emit('S_binary', [bAns,className]);
	activeBinary=0;
	}
	else{
...
const MCQUpdate =(val)=>{
	...
	socket.emit('S_MCQ', [MCQAns,className]);
...
className="AP-300"    //Wasif change here
activeBinary=0;
activeMCQ=0;
activeCp=0;							//Can post cp twice
socket.emit('Student',["",className]);     //To notify the server that student got connected So that server can discriminate b/w users
// setState();
